...
class PhotoRepository
{
    ...
    public function findSharedByToken(string $token): array
    {
        ...
        $allowedCategories = array_map('intval', $shareMap['categories']);
    ...
    public function findOneForShare(int $photoId, string $token): ?array
    {
        $shareMap = $this->shareMetaByToken($token);
        if (!$shareMap) {
            return null;
        ...
        $hydrated = $this->hydrateCategories([$photo]);
        $photo = $hydrated[0] ?? null;
        if (!$photo) {
            return null;
        }

        if (!empty($allowedCategories)) {
            $photoCategories = array_map('intval', $photo['category_ids'] ?? []);
            $hasMatch = false;
            foreach ($photoCategories as $categoryId) {
    ...
    public function shareMetaByToken(string $token): ?array
    {
        ...
        $shareMap['id'] = (int)$shareMap['id'];
        $shareMap['user_id'] = (int)$shareMap['user_id'];
        $shareMap['categories'] = array_map('intval', $this->categories->findShareCategoryIds((int)$shareMap['id']));
