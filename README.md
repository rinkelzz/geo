...
3. Installations-/Upgrade-Assistent im Browser ausführen und Konfiguration speichern:
   ...
   Rufe anschließend `http://localhost:8000/install.php` auf, trage deine Datenbank-Zugangsdaten ein, hinterlege eine Absender-Adresse für Benachrichtigungs-Mails und starte die Schema-Installation. Nach erfolgreicher Einrichtung sollte `public/install.php` aus Sicherheitsgründen gelöscht werden.

   - In `config/config.php` kannst du anschließend Pfade und Limits wie `upload_dir`, `original_upload_dir` oder `display_max_dimension` sowie weitere Mail-Einstellungen (Absendername, Admin-Verteiler) nach Bedarf anpassen.
...
4. Upload- und Shared-Verzeichnisse (inklusive Originalbildern) sicher beschreibbar machen:
   ...
   > ```
   >
   > Ergänze in `config/config.php` außerdem einen neuen Abschnitt `mail` mit `from_address`, optionalem `from_name` sowie den gewünschten Admin-Empfänger:innen, damit Benachrichtigungen versendet werden können.
