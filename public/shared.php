...
$photos = array_map(static function (array $photo) use ($token) {
    $photoId = (int) $photo['id'];
    $query = http_build_query(['id' => $photoId, 'token' => $token]);
    $photo['image_url'] = '/photo.php?' . $query;
    $photo['original_image_url'] = !empty($photo['original_file_path'])
        ? '/photo.php?' . http_build_query(['id' => $photoId, 'variant' => 'original', 'token' => $token])
        : $photo['image_url'];
    return $photo;
}, $photos);
$photosForJs = $photos;
        ...
        <?php if ($hasPhotos): ?>
            <?php foreach ($photos as $photo): ?>
                ...
                <article class="photo-card" data-photo-card data-photo-id="<?php echo (int)$photo['id']; ?>" tabindex="0" role="button" aria-label="Foto-Details anzeigen: <?php echo $photoTitleAttr; ?>">
                    <img src="<?php echo htmlspecialchars($photo['image_url'], ENT_QUOTES); ?>" alt="<?php echo htmlspecialchars($photo['title'] ?? 'Foto', ENT_QUOTES); ?>">
