...
$photos = array_map(static function (array $photo) {
    $photoId = (int) $photo['id'];
    $baseQuery = http_build_query(['id' => $photoId]);
    $photo['image_url'] = '/photo.php?' . $baseQuery;
    $photo['original_image_url'] = !empty($photo['original_file_path'])
        ? '/photo.php?' . http_build_query(['id' => $photoId, 'variant' => 'original'])
        : $photo['image_url'];
    return $photo;
}, $photos);
